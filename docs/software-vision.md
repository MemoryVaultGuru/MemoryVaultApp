...
- **The reserved vocabulary comes along** (RN-AGT-025), read from the pinned specification. Without it an agent has no way to tell an attribute name that means the same thing in every vault from one that belongs to this vault alone, and the difference decides whether writing `autor:` or `author:` is a choice or a mistake. The section says what `author` and `co-author` mean here, and that the product teaches the convention and never writes the key.
